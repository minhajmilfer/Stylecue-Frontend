...
// Every key we've saved to the shared notebook (localStorage) throughout
// the visit - cleared once the order is confirmed so the next customer
...
export default function ConfirmedPage() {
  ...
  useEffect(() => {
    ...
    const savedRequestId = localStorage.getItem("stylecue_request_id");
    setRequestId(savedRequestId);

    // The customer's visit is over - end their session on the backend so
    // their measurements and preferences are forgotten, per the original
    // privacy design (no data should persist after a session ends).
    if (sessionId) {
      fetch(`${API_URL}/sessions/${sessionId}`, { method: "DELETE" }).catch((err) => {
        // Not critical to the customer's experience if this fails silently -
        // they've already gotten their confirmation either way.
        console.error("Could not end session cleanly:", err);
      });
    }

    // Clear the shared notebook so the next customer starts fresh.
        ...
        <div className="w-full bg-purple-950/30 backdrop-blur-xl border border-purple-500/20 rounded-2xl p-4 md:p-5 shadow-2xl space-y-3">
          ...
          <div className="space-y-2 text-left max-w-xs mx-auto pt-1">
            ...
            <div className="flex items-center gap-3 text-xs font-medium">
              ...
              <span className="text-amber-300 font-semibold">2. Staff preparing your items</span>
            </div>
